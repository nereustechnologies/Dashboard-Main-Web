...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    try {
      ...
      // Check if this user has permission to download this test
      // Testers can download their own tests, admins can download any test
      if (test.testerId !== user.id && user.role !== "admin") {
        return new NextResponse("You do not have permission to download this test", { status: 403 })
      }
